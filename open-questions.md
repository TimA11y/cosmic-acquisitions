...
None remaining. All four are resolved:

- ~~No self-hosted display typeface.~~ **Fixed.** Every heading (`h1`–`h4`, site-wide) now uses **Orbitron**, a self-hosted geometric/angular display face (SIL Open Font License, `fonts/OFL.txt`), while body text stays on the system stack. Shipped as a ~6KB bold-weight, printable-ASCII-only subset (`fonts/orbitron-bold-subset.woff2`) of the upstream variable font, `font-display: optional` (no visible FOUT). Verified loading correctly over a genuine `file://` open — font resource loads aren't blocked by `file://`'s same-origin restriction the way ES module imports are, so this doesn't compromise R3.
